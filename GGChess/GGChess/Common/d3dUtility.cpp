	...
	IDirect3DDevice9** device)
{
	...
	wc.lpszMenuName  = 0;
	wc.lpszClassName = A2W("Direct3D9App");
	...
	HWND hwnd = 0;
	hwnd = ::CreateWindow(A2W("Direct3D9App"), A2W("Direct3D9App"), 
		WS_EX_TOPMOST,
		0, 0, width, height,
...
bool d3d::ComputeBoundingSphere(ID3DXMesh* mesh, d3d::BoundingSphere* sphere)
{
	HRESULT hr = 0;
...
bool d3d::ComputeBoundingBox(ID3DXMesh* mesh, d3d::BoundingBox* box)
{
	HRESULT hr = 0;
...
bool d3d::LoadX(IDirect3DDevice9 *Device,
				const char *Name,
				ID3DXMesh **Mesh,
...
void d3d::ComputeNormal(D3DXVECTOR3 *p0, D3DXVECTOR3 *p1,	
...
d3d::Ray d3d::CaclulatePickingRay(IDirect3DDevice9* device, int x, int y) {
	float px = 0.0f;
	float py = 0.0f;
	...
	px = ((( 2.0f*float(x)) / float(vp.Width )) - 1.0f) / proj(0, 0);
	py = (((-2.0f*float(y)) / float(vp.Height)) + 1.0f) / proj(1, 1);

	d3d::Ray ray;
	ray.origin = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	ray.direction = D3DXVECTOR3(px, py, 1.0f);

	// transform it according to the view transformation.
	D3DXMATRIX viewInverse;
	device->GetTransform(D3DTS_VIEW, &viewInverse);
