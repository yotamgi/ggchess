...
FPSCounter::FPSCounter(IDirect3DDevice9* device)
{
	...
	_font = new CD3DFont((const TCHAR*)"Times New Roman", 24, 0);
...
bool FPSCounter::render(D3DCOLOR color, const char *String)
{
	USES_CONVERSION;
	_font->DrawTextW(20, 20, color, (const TCHAR*)A2W(String));	
...
bool FPSCounter::render(D3DCOLOR color, float timeDelta)
{
	...
	if( _font )
	{
		...
		_font->DrawTextW(20, 20, color, (const TCHAR*)A2W(_fpsString));	
	}
	return true;
}